...
class Grid:
    ...
    def LowestEntropy(self):
        ...
        logger.debug("Cell with lowest entropy: {}", lowestEntropyIndex)
    ...
    def UpdateCellOptions(self, cellIndex, availableOptions):
        row = cellIndex[0]
        column = cellIndex[1]
        logger.debug("Available options: {}", availableOptions)
        logger.debug("My options: {}", self.Cells[row][column].options)
        ...
        if self.Cells[row][column].isCollapsed():
            ...
            logger.debug("Cell [", row, "][", column,
                         "]. My new options: ", self.Cells[row][column].options)
